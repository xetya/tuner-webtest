// Activated for the standalone web build: originally wrapped in an
// unclosed /* comment (dead code) upstream. Only i18n.en strings are
// used by assets/js/app.js (small UI hints); other languages are kept
// as-is from the source dump but are not wired into the UI yet.
// `sDeviceFamily` normally comes from the native app's bootstrap layer,
// which this standalone build doesn't have — the file's last block reads
// it, so it's declared here to avoid a ReferenceError.
var sDeviceFamily = 'Browser';
